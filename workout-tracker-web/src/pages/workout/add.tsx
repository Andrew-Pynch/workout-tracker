...
import { LoginButton, PrimaryButton } from "~/components/themed/CustomButtons";
...
const Add = (props: AddProps) => {
  ...
  const { data: exercisesByBodyGroup } =
    api.exercise.getNamesForUserByBodyGroup.useQuery({
      bodyGroup: bodyGroup,
    });

  const { data: mostRecentExercisesByExerciseAndBodyGroup } =
    api.exercise.getNMostRecentForUserByExerciseAndBodyGroup.useQuery({
      bodyGroup: bodyGroup,
      exercise: exercise,
      n: 3
    });
  ...
  const postExercise = api.exercise.add.useMutation({
    onMutate: async (newEntry) => {
      await utils.exercise.getNMostRecentForUserByExerciseAndBodyGroup.cancel();
    },
    onSettled: async () => {
      await utils.exercise.getNMostRecentForUserByExerciseAndBodyGroup.invalidate();
      setBodyGroup("");
      setExercise("");
      setSets(0);
      setReps(0);
